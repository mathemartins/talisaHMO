...
from insuranceplans.models import Insuranceplan
from talisa.utils import unique_order_id_generator
...
class OrderManagerQuerySet(models.query.QuerySet):
    ...
    def cart_data(self):
        return self.aggregate(
            Sum("cart__insuranceplans__price"),
            Avg("cart__insuranceplans__price"),
            Count("cart__insuranceplans")
        )
...
class Order(models.Model):
    ...
    def update_purchases(self):
        for p in self.cart.insuranceplans.all():
            obj, created = InsuranceplanPurchase.objects.get_or_create(
                order_id=self.order_id,
                insuranceplan=p,
                billing_profile=self.billing_profile
            )
        return InsuranceplanPurchase.objects.filter(order_id=self.order_id).count()
...
class InsuranceplanPurchaseQuerySet(models.query.QuerySet):
    def active(self):
        return self.filter(refunded=False)

    def digital(self):
        return self.filter(insuranceplan__is_digital=True)
...
class InsuranceplanPurchaseManager(models.Manager):
    def get_queryset(self):
        return InsuranceplanPurchaseQuerySet(self.model, using=self._db)
    ...
    def insuranceplans_by_id(self, request):
        qs = self.by_request(request).digital()
        ids_ = [x.insuranceplan.id for x in qs]
        return ids_

    def insuranceplans_by_request(self, request):
        ids_ = self.insuranceplans_by_id(request)
        insuranceplans_qs = Insuranceplan.objects.filter(id__in=ids_).distinct()
        return insuranceplans_qs


class InsuranceplanPurchase(models.Model):
    order_id = models.CharField(max_length=120)
    billing_profile = models.ForeignKey(BillingProfile, on_delete=models.CASCADE)  # billingprofile.insuranceplanpurchase_set.all()
    insuranceplan = models.ForeignKey(Insuranceplan, on_delete=models.CASCADE)  # insuranceplan.insuranceplanpurchase_set.count()
    refunded = models.BooleanField(default=False)
    updated = models.DateTimeField(auto_now=True)
    timestamp = models.DateTimeField(auto_now_add=True)

    objects = InsuranceplanPurchaseManager()

    def __str__(self):
        return self.insuranceplan.title
