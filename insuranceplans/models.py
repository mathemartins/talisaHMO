...
from talisa.settings.aws.download.utils import AWSDownload
from talisa.settings.aws.utils import ProtectedS3Storage
from talisa.utils import unique_slug_generator, get_filename
...
def upload_image_path(instance, filename):
    ...
    return "insuranceplans/{new_filename}/{final_filename}".format(
        new_filename=new_filename,
        final_filename=final_filename
    )


class InsuranceplanQuerySet(models.query.QuerySet):
    def active(self):
...
class InsuranceplanManager(models.Manager):
    def get_queryset(self):
        return InsuranceplanQuerySet(self.model, using=self._db)

    def all(self):
        return self.get_queryset().active()

    def featured(self):  # Insuranceplan.objects.featured()
        return self.get_queryset().featured()

    def get_by_id(self, id):
        qs = self.get_queryset().filter(id=id)  # Insuranceplan.objects == self.get_queryset()
        if qs.count() == 1:
            return qs.first()
        return None
...
class Insuranceplan(models.Model):
    ...
    def get_absolute_url(self):
        # return "/insuranceplans/{slug}/".format(slug=self.slug)
        return reverse("insuranceplans:detail", kwargs={"slug": self.slug})
...
pre_save.connect(insuranceplan_pre_save_receiver, sender=Insuranceplan)


def upload_insuranceplan_file_loc(instance, filename):
    slug = instance.insuranceplan.slug
    # id_ = 0
    id_ = instance.id
    if id_ is None:
        ...
        if qs.exists():
            ...
            id_ = 0
    if not slug:
        slug = unique_slug_generator(instance.insuranceplan)
    location = "insuranceplan/{slug}/{id}/".format(slug=slug, id=id_)
    return location + filename  # "path/to/filename.mp4"


class InsuranceplanFile(models.Model):
    ...
    def get_default_url(self):
        return self.insuranceplan.get_absolute_url()
    ...
    def generate_download_url(self):
        ...
        if not secret_key or not access_key or not bucket or not region:
            return "/insuranceplan-not-found/"
    ...
    def get_download_url(self):  # detail view
        return reverse("insuranceplans:download",
                       kwargs={"slug": self.insuranceplan.slug, "pk": self.pk}
                       )
